/**
 * **실제 기획 메모 하나를 자연어 초안부터 게시까지 통째로 보낸다.**
 *
 * `test:demo`(§20 대본)와 다른 점은 **입력의 크기**다. 대본 픽스처는 숙소 2행·상점
 * 3행이지만 실제 메모는 **숙소 2곳 + 카페·음식점 13곳 + 여행지 포인트 7곳**이다.
 * 검증 2종(`fact-check`)은 `form_input`을 통째로 프롬프트에 싣기 때문에(§11.1)
 * 그 크기가 25~40초 예산에 그대로 얹힌다 — **그 구간이 아직 미실측이었다.**
 *
 *   npm run dev  (별도 터미널)
 *   npm run test:real
 *
