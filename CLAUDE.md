...
| 문서 | 지위 |
|---|---|
| **`spec.md` (2.8)** | **구현의 유일한 권위.** 스키마·상한·규정·응답코드 전부. 2.8은 상품 페이지의 고정 9섹션을 없애고 구성·순서·분위기를 AI가 짜게 한 판본이다(§8) |
...
| `docs/checklist-verdict.md` | **`checklist.md` 355항목의 판정 기록** (✅❌⚠️📦🚫 · 게이트 G0~G6). 「이 항목은 이미 봤나」의 답이 여기 있다. `docs/responsive-verification.md`(375/768/1280 검사 방법)를 여기서 가리킨다. ⚠️ **2.7 배열화·2.8 자유구성화 이후의 355항목 재판정은 아직 없다** |
...
| **`docs/harness-skill-map.md`** | **🗺 스킬·에이전트 지도 (사람용).** 무엇이 실제로 돌고(28개) 무엇이 `kind:spec` 설명서라 안 도는지(4개)·왜 지우면 안 되는지. manifest를 풀어 쓴 것 |
...
| **`docs/harness-migration.md` §10** | **🔧 8/13 실사용 기록 — 여기부터 읽는다.** §10.6 AI 경로 차단 해제 · §10.7 타임아웃 55초 상향 · **§10.8 사용자 테스트 결함 3건**(대시 없는 라벨·사람이 쓴 일정 보존·**행사기간을 여행기간과 분리** — 선택 신규 필드) |
| **`docs/harness-migration.md` §9** | **📐 spec 2.8 설계 (2) — 채택 안 됨.** 필수 입력을 「일정」에서 「의도」로 바꾸자는 안(`기획메모` 필수·최상단, 일정 선택, `장소[]` 신설). **§8은 이것 없이 나갔다** — spec 2.8은 `기획메모`를 선택·고객미노출 참고 필드로만 뒀고(§6.1) `일정`을 그대로 유지했다. 분위기 판단 근거는 기존 `여행스타일`·`타겟층`·`여행주제`가 댄다. 이 절은 다시 손댈 때의 대안 설계로만 남는다 |
| **`docs/harness-migration.md` §8** | **✅ spec 2.8 설계 — 구현됨.** 상품 페이지의 고정 형식 9섹션을 없애고 구성·순서·분위기를 AI가 짜게 한 것. 값 치환은 기계가 유지하고 검증은 「섹션 대응」 → 「`source` 커버리지」로 옮겼다. 코드에 반영됐다 — `content-structuring`(COMPOSE)·`block-vocabulary-gate`·`web-content-structure-gen`, manifest `2.8.0`. **이 절은 그 설계 근거**이고, 규정은 `spec.md` 2.8이 갖는다 |
...
| **AI 공급자** | `claude-opus-5` (2.4까지) | **`gemini-3.5-flash-lite` 단일 공급자** (2.6), `lib/ai` provider 중립 인터페이스 | §4.3 |
...
| **effort** | 2종 (생성 medium / 검증 low) | **3종.** 초안은 `thinkingLevel: LOW`(Gemini엔 사고를 끄는 값이 없어 최선) | §4.3 |

## 🔒 하네스 규약 — 절대 불가침

**이 규약은 런타임 실행과 코드 작성 양쪽에 동시에 적용된다. 예외는 없다.**
일정 압박·버그 급함·"이번 한 번만"은 예외 사유가 아니다. 규약을 어겨야 풀리는 문제라면
...
1. **1요청 1AI호출.** 서버 라우트 1건은 AI를 최대 1회. 타임아웃 **55초**, SDK 자동 재시도 없음, `maxDuration` 60초 (§4.2·§4.3).
   **하네스 규약 R3이 `ai_budget`으로 기계 강제한다**
2. **재시도는 클라이언트가 같은 API를 재호출.** 서버 내부 루프·폴링·큐·Cron 없음. `202` 사용 안 함
3. **AI 출력은 JSON 스키마로 강제.** Gemini `responseSchema`로 제공자가 강제하고, **`lib/ai/schema.ts` 서버 검증**이 방어층으로 한 번 더 관문을 세운다. 어긋난 구조가 파이프라인에 들어가는 경로는 없다 (§4.3)
...
8. **모델은 `gemini-3.5-flash-lite`** (Gemini, `@google/genai` SDK). effort 생성 `medium`/검증 `low`/초안 `low`,
   타임아웃 **55초**, SDK 자동 재시도 없음 (§4.3). 막히면 `AI_MODEL`로 다른 flash 계열(예 `gemini-2.5-flash`)로 교체.
   **라우트는 `lib/ai`의 provider 중립 인터페이스만 호출한다** — 모델을 바꿔도 라우트는 수정하지 않는다

## 목표

종료 조건은 **spec §20의 엔드투엔드 시나리오**를 처음부터 끝까지 관통하는 것이다.
로그인 → `/new` 폼+이미지 → [소개서 생성](4요청) → [상품 생성](3요청) → 편집기 → [게시] → 비로그인 `/p/{slug}` → 신청 → 이메일 → `/admin/logs/{execution_id}`.

측정 기준은 §1 — 입력부터 배포까지 10분, 코드·디자인 작업 0건.
...
`probe:gemini`는 **작은 샘플 입력**(795~1105 토큰)을 보낸다. 실제 라우트는 `form_input` ·
`confirmed_data` · `page_content`를 통째로 직렬화해 보내므로 입력이 훨씬 크고, **그만큼 느리다.**
아래 두 열은 같은 날(2026-08-12) 같은 모델로 잰 값이다.

| 호출 | `probe:gemini` | 2.6 | **소규모 입력** (숙소 2·상점 3) | **실제 메모** (숙소 2·상점 13) | 40초 여유 |
|---|---:|---:|---:|---:|---:|
...
| 자연어 초안 | — | — | — | **3.1초** (초안 effort) | 36.9초 |

**실제 메모 열이 기준이다.** `npm run test:real`로 잰 값이며(상호 15곳 · `form_input` 1611자),
자연어 초안부터 게시까지 **114초 · 18 통과 · 0 실패**로 관통했다. 소규모 입력만 보고
...
- ⚠️ **그래서 타임아웃을 25초 → 40초로 올렸다**(2.7). 25초에서는 분해가 19.2 / 21.9 / 25.6초로
  ...
  타임아웃의 80%라는 연동을 끊었다. 그 플래그는 파이프라인이 노출하는 관측치다
- ⚠️ **분해가 35.3초로 40초에 4.7초만 남긴다.** 상점이 더 많은 메모(20곳 이상)는 재시도에
  의존하게 된다. 그 경로는 정상 동작이지만(§11.6) 상한 안에 들어오는지는 다시 재야 한다.
...
- 분해가 3회 연속 타임아웃하면 422가 된다. 프롬프트 축소 전에 실제로 관측됐다(25.8·26.0·25.7초)
  관통 테스트에서 실제로 `validate-page`가 409 후 재시도로 통과한 적이 있다.
  **재시도 경로가 그 상황을 위해 있으므로 고장이 아니다** — 재호출로 관통한다
- **프롬프트를 늘리면 이 여유를 깎는다.** user 메시지 지시문을 SKILL.md로 옮기는 작업이
  뒤로 미뤄진 이유가 이것이다 — 옮긴 뒤 이 표를 **다시 재야 한다**
- `npm run dev` 첫 요청은 라우트 컴파일 때문에 38초까지 나온다. 프로덕션 빌드에는 없는 비용이다.
  **측정 전에 라우트를 한 번씩 예열하면** 관통 2:22 → 1:28로 줄어든다(실측)
- **빈 본문이 관측될 수 있다** (종료 사유 `stop`인데 본문만 빔).
  `schema_invalid` → §11.6 재시도 경로로 처리된다. 재호출로 해소됨을 실측
- **시스템 프롬프트는 바이트 단위로 고정한다** — 실측 재현 조건을 유지하고
  Gemini 유료 티어의 컨텍스트 캐시(무료 티어엔 암묵 캐싱 없음) 프리픽스를 살린다
- **공급자는 Gemini 하나뿐이다.** 무료 티어는 모델당 하루 20회(관통 1회가 AI 6회)라
  일일 한도는 대기로 회복되지 않는다. 상시 사용은 결제 계정 키를 전제한다

### Resend 발신 제약 (확정)

- **Resend 도메인 미인증으로 간다** (§13.3이 규정한 경로). 발신은 `onboarding@resend.dev` 고정이고
  **수신은 Resend 가입 이메일 주소로만** 된다. 따라서 신청 폼 이메일에는 **본인 가입 이메일**을 넣어야 도착을 확인할 수 있다.
...
npm run test:harness     # 🔒 하네스 규약 위반 검사. 커밋 전 필수 (185 · 0 · 미구현 0)
npm run test:policy      # 규칙·계약 회귀 (316 · 0)
...
npm run test:real        # §20 엔드투엔드 관통(실제 메모). dev 서버 필요 · **AI 6회 + 이메일 1통**
npm run test:plan-draft  # §7.5 자연어 초안 관통. 실제 기획 메모 1건 · **AI 2회** (18 · 0)
npm run test:exhaustion  # 재시도 소진 경로. AI를 일부러 실패시킨 서버가 필요하다
```

`test:exhaustion`은 `KEEP=1`을 붙이면 데이터를 남긴다(기본은 지운다). `test:real`은 브라우저 확인용이라 기본으로 남긴다.
