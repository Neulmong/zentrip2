...
| **`docs/harness-migration.md` §10** | **🔧 8/13 실사용 기록 — 여기부터 읽는다.** §10.6 DeepSeek 차단 해제 · §10.7 타임아웃 55초 상향 · **§10.8 사용자 테스트 결함 3건**(대시 없는 라벨·사람이 쓴 일정 보존·**행사기간을 여행기간과 분리** — 선택 신규 필드) |
...
8. **모델은 `deepseek-v4-flash`** (DeepSeek 종량제, OpenAI 호환 SDK). `reasoning_effort` 생성 `medium`/검증 `low`,
   타임아웃 **55초**, `maxRetries: 0` (§4.3). **`deepseek-v4-pro`는 쓰지 않는다** — `AI_MODEL`에 pro·비flash 값이
