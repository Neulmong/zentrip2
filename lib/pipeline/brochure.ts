  ...
  return {
    ...
    sections: [
      ...
      },
      {
        ...
        data: {
          // 행사기간은 선택 — 미입력이면 빈 문자열이며 렌더러가 표시하지 않는다.
          // 데이터 모델에는 항상 실어 source 대조를 균일하게 유지한다(빈 값 = 빈 값).
          여행지: g.여행지, 행사기간: g.행사기간, 여행기간: g.여행기간,
          타겟층: g.타겟층, 여행스타일: g.여행스타일,
          여행주제: g.여행주제,
          핵심일정,
        },
        source: {
          여행지: '행사정보.여행지', 행사기간: '행사정보.행사기간', 여행기간: '행사정보.여행기간',
