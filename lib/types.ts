...
// ── AI 호출 계약 (§4.3) ─────────────────────────────────────────
/**
 * 모델 이름은 **여기에 두지 않는다.** 공급자는 Gemini
 * (`gemini-3.5-flash-lite`)이고, 라우트는 `lib/ai`의 provider 중립 인터페이스만
 * 호출한다 — 기본 모델은 `lib/ai/gemini.ts`가, 교체는 `AI_MODEL` 환경 변수가
 * 담당한다. 2.4까지 있던 `AI_MODEL = 'claude-opus-5'` 상수는 아무도 읽지
 * 않으면서 「모델이 여기 적혀 있다」는 오해만 남겨 폐기했다. 공급자가 여러 번
 * 바뀌는 동안 이 파일이 한 번도 안 바뀐 것이 그 판단의 근거다.
 */
 ...
 *
 * ## 40초 → 55초 (2026-08-13 · 라이브 재실측 근거)
 *
 * 라이브 실측에서 분해가 **45~47초로 40초를 상습적으로 넘겼다** — `test:cases`
 * 골든 스위트에서 G01 39.5초(턱걸이 통과) · G02 46·47·47초(3회 초과 → 422 실패) ·
 * G03 45.9초(재시도 후 통과). 40초는 분해 지연 분포(19~47초)의 중앙보다
 ...
 * 더 이상 플래그를 남기지 않는데, 그 플래그는 **없애야 할 값이 아니라 파이프라인이
 * 노출하는 관측치**다(§5.5).
 */
