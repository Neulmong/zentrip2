...
export const PROMPTS = {
  ...
  "intro-content-fill": "너는 여행 상품 소개서의 개요 문장을 쓴다.\n\n「핵심일정」은 이 여행이 각 일차에 주로 도는 곳을 순서대로 짚고, 여행의 테마를 설명하는\n2~3문장이다. 아래 형식을 따른다:\n  「이 여행은 1일차에는 (지역/장소), 2일차에는 (지역/장소), 3일차에는 (지역/장소) 위주로\n   도는 (테마) 테마의 여행 상품입니다.」\n- 각 일차는 그 날 일정에 등장한 장소를 대표하는 **지역이나 핵심 장소 한둘**로 짚는다.\n  그 날의 장소들이 한 지역(예: 성산·서귀포·삼양 일대)에 모여 있으면 그 지역명으로 짚고,\n  지역이 분명하지 않으면 그 날의 대표 장소 이름을 그대로 쓴다.\n- 테마는 여행스타일·여행주제에서 온다(관광·미식·휴양·힐링·자연 등). 한두 낱말로 압축한다.\n- 마지막에 여행의 결을 설명하는 한 문장을 덧붙여도 좋다(선택).\n\n절대 규칙:\n- 일정에 없는 장소·지역·활동을 추가하지 않는다. 일정에 등장한 장소가 속한 지역만 짚는다.\n- 확실하지 않은 지역명을 지어내지 않는다 — 모르면 그 날의 대표 장소 이름을 그대로 쓴다.\n- 출처 없는 숫자를 만들지 않는다. 소요 시간·거리·인원을 추정하지 않는다.\n- 가격을 계산·합계·환산하지 않는다.\n- 고유명사의 표기를 바꾸지 않는다. 약칭·영문 변환을 하지 않는다.\n- 마케팅 문구·과장 표현을 쓰지 않는다.\n- 존댓말로 쓴다. 종결어미는 ~습니다/~입니다로 통일한다.\n- 중괄호 토큰이나 파이프 기호를 출력에 남기지 않는다.\n\n「기획 메모」가 주어지면 **어조를 잡는 참고 자료로만** 쓴다.\n그 내용을 출력에 인용하거나 사실로 옮겨 적지 않는다 — 고객에게 표시되지 않는\n내부 메모이며, 거기 적힌 나이·인원·인물·가격은 **사실정보가 아니다.**\n메모를 읽고 「누가 읽을 글인가」만 감을 잡은 뒤, 문장은 확정 데이터만으로 쓴다.",
...
export const PROMPT_HASHES = {
  ...
  "intro-content-fill": "1e92594c19f5",
...
export const USER_PROMPTS = {
  ...
  "intro-content-fill": {
    "default": "아래 일차별 서술과 여행스타일·여행주제를 근거로 「핵심일정」을 위 형식(각 일차가 도는 지역/장소 + 테마)의 2~3문장으로 써라.",
  },
