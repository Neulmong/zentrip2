...
npm run test:harness  149 통과 · 0 실패 · 1 격차   (8/12 재검증: 103 → 149)
...
**남은 3건은 전부 「AI에게 보내는 바이트가 바뀌는」 변경이다.** 그래서 하나로 묶인다 —
손대면 `probe:deepseek`과 §7.3 ④의 파이프라인 소요 시간표를 **함께 다시 재야 한다.**
일차 분해가 25초 예산에 2.5초 남기고 붙어 있으므로, 프롬프트가 길어지면 그 여유가 줄고
409 재시도 빈도가 올라간다.

| 항목 | 무엇이 걸리나 | 얻는 것 |
|---|---|---|
| user 메시지 지시문을 SKILL.md로 (`test:harness`의 ⏳ 1건) | SKILL.md에 **user 템플릿 펜스**와 보간 규칙을 새로 만들어야 한다. system은 고정 문자열이라 쉬웠지만 user는 요청마다 값이 달라진다 | R4가 완결된다. AI에게 가는 문장이 전부 문서에서 온다 |
| spec §6.3 판정 3단계 (AI가 명사구 후보를 판정) | `DECOMPOSE_SCHEMA`에 자기검증 필드가 붙고 프롬프트가 바뀐다 → 캐시 적중·바이트 동일이 깨진다 | 0차가 spec대로 3단계까지 간다. 지금은 2단계까지 하고 후보를 로그에만 남긴다 |
| `consistency-check`를 mechanical로 | 3차 축이 「AI 의미 대조」에서 「기계 값 대조」로 성격이 바뀐다. 페이지가 소개서 값을 코드로 승계하므로 기계 대조가 가능하긴 하다 | AI 6 → 5회. 예산 여유·비용·소요 시간이 모두 준다 |

세 번째를 먼저 하면 예산 여유가 생겨 앞의 둘이 안전해진다 — **순서는 3 → 1 → 2**를 권한다.

창작 방어는 §6.3 3단계가 유일한 방어선이 아니다. 1·2차 `fact-check`가
「입력에 없는 지명·시설·경유지·관광지 등장」을 실패로 잡는다.
