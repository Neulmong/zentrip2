...
## 10.6 ⛔ 차단 해제 + DeepSeek 라이브 재실측 (2026-08-13 오후)

**§10.3의 내부망 차단이 풀렸다.** DeepSeek `deepseek-v4-flash`가 다시 연결되고,
막혀서 못 돌리던 AI 관통 5종을 전부 돌렸다 — **전부 통과다.** (`AI_PROVIDER`·`AI_MODEL`
비워 둔 기본 경로)

| 테스트 | 결과 | 비고 |
|---|---|---|
| `probe:deepseek` | ✅ | 분해 19.1초 · 페이지확장 34.4초 · 검증 1.1초 |
...
이번 실행에서 `probe:deepseek`이 페이지확장 34.4초를 **「25초 예산 초과」**로 찍었다.
...
### 남는 관측 — DeepSeek 분해 지연이 시간대별로 크게 흔들린다
