...
| Gemini | 6곳 — `/decompose`·`/brochure`·`/validate-brochure`·`/page`·`/validate-page`·`/validate-consistency` |
...
2. **실측 재현성** — 시스템 프롬프트는 요청 간 **바이트 단위로 동일**해야 한다. 그래야 실측 조건이 흔들리지 않고, 유료 티어로 올릴 때 Gemini 컨텍스트 캐시 프리픽스도 살아난다(무료 티어엔 암묵 캐싱이 없다). 동결된 문자열 상수는 이를 보장하고, 파일 읽기는 EOL·인코딩 드리프트 위험을 안는다. 코드젠은 CRLF를 `\n`으로 정규화해 프롬프트 바이트를 고정한다.
...
- **첫 전환 커밋에서 프롬프트 바이트는 정확히 동일해야 한다.** 문면을 다듬는 것은 데모 후다.
  현재 프롬프트로 §20 대본 통과와 실측 재현이 확인됐다(프롬프트 바이트 고정은 실측 재현·Gemini 유료 티어 캐시를 위한 것). 하네스 전환과 프롬프트 변경을
...
실측 재현성이 유지된다(프롬프트 바이트 고정). 고친 것은 전부 사람이 읽는 산문·표다.
...
이전 판본은 셋 다 캐시 적중을 깨므로 `probe:gemini` 재실측이 필요하다고 적었다.
**컨텍스트 캐시(Gemini 유료 티어)는 최장 공통 접두를 잡고 system이 앞에 온다** — user 쪽 변경은
...
409를 낸다.** 표는 spec §4.3 · `lib/ai/gemini.ts`의 `THINKING`.
...
여기까지는 **키 없이도 돈다.** AI가 필요한 것은 `probe:gemini`·`test:real`뿐이다.

```bash
npm run dev                          # 별도 터미널
npm run probe:gemini                 # 주 AI 경로 실측 — 키를 건드렸으면 이것부터
npm run test:real                    # §20 엔드투엔드 관통. AI 6회를 쓴다
```

⚠️ `test:real`은 **실제 AI를 6회 호출하고 이메일을 1통 보낸다.** 수신은 Resend 가입
주소로만 도달한다.
...
| 프롬프트 해시가 바뀐다 | `itinerary-decomposition`의 시스템 프롬프트를 고치므로 컨텍스트 캐시 프리픽스(Gemini 유료 티어)가 새로 잡힌다. **입력이 커지므로(기획메모 + 장소 배열) 35.3초였던 분해 시간을 다시 재야 한다** — 40초 예산에 4.7초만 남아 있었다 |
...
이전 AI 공급자에 연결이 안 되는 상태에서 SDK가 «Request timed out.»을 던지는데 실제 경과는
...
## 10.3 ⛔ 차단 — 내부 네트워크가 이전 AI 공급자를 막는다

> **2026-08-13 오후 해제됨 — §10.6.** 아래는 차단이 살아 있던 동안의 기록이다.

**코드 문제가 아니다.** 기획자 확인: 내부 네트워크에서 이전 AI 공급자 접근을 차단한다.

```
DNS         이전 공급자 API 엔드포인트 → CloudFront, 정상 해석
...
**일반 외부 HTTPS는 정상이고 이전 AI 공급자만 막혔다.** 그 상태에서 분해가 3회 소진되어
...
## 10.6 ⛔ 차단 해제 + AI 경로 라이브 재실측 (2026-08-13 오후)

**§10.3의 내부망 차단이 풀렸다.** AI 주 경로가 다시 연결되고,
막혀서 못 돌리던 AI 관통 5종을 전부 돌렸다 — **전부 통과다.** (`AI_MODEL`
비워 둔 기본 경로)

| 테스트 | 결과 | 비고 |
|---|---|---|
| `probe:gemini` | ✅ | 분해 19.1초 · 페이지확장 34.4초 · 검증 1.1초 |
| `test:plan-draft` | 18 · 0 | 자연어 초안 (AI 2회) |
| `test:real` | 18 · 0 · **204초** | 자연어→게시 관통 (상호 15곳) |
| `test:demo` | 48 · 0 | §20 대본 · **실제 이메일 1통**(delivered@resend.dev) · 로그 16행 |
| `test:pipeline` | 47 · 0 | 4축 전부 pass |

키 불필요 기준선도 재확인: `test:harness` **181 · 0 · 미구현 0** · `test:policy` **294 · 0**.

### §10.4가 라이브에서 확정됐다 — 분해가 실제로 40초를 넘는다

「분해가 40초 예산에 붙어 있다」가 이번엔 실측으로 재현됐다. `test:real`에서 일차 분해가
**41.7초 · 40.9초로 2회 연속 타임아웃(409 retry) → 3번째 21.3초에 통과**했다. §11.6
재시도 경로가 설계대로 받아냈으므로 고장은 아니지만, **그 3회가 ~104초를 먹는다.**
`probe`의 페이지확장(34.4초)도 40초에 5.6초만 남긴다.

그래서 `test:real` 총 시간이 2.7의 114초에서 **204초**로 늘었다 — 차이의 대부분이 분해
2회 재시도(~83초)와 cold 라우트 컴파일이다. **§20 대본 3:00 안에 든다는 보장이 이제
없다.** 데모 전에 §10.4의 선택지(타임아웃 40→50초 · 프롬프트 축소 · 재시도에 맡김) 중
하나를 **사람이 정해야 한다** — spec §4.2·§4.3·checklist A-08의 「40초」를 건드리는
결정이라 코드가 임의로 올리지 않았다.

### probe 라벨 결함 (함께 고침)

이번 실행에서 `probe:gemini`가 페이지확장 34.4초를 **「25초 예산 초과」**로 찍었다.
2.7에서 타임아웃을 40초로 올렸는데 probe의 임계값이 25초로 남아 있어 40초 예산 안(34.4초)을
초과로 오표시했다. `25_000` → `40_000`, 라벨 문구도 40초로 고쳤다. (§10.7에서 55초로 다시 올렸다.)

---

## 10.7 타임아웃 40 → 55초 + AI 파이프라인 케이스 스위트 (2026-08-13 저녁)

§10.4·§10.6이 남긴 「분해가 40초에 붙어 있다」 판단을 **케이스 스위트로 강제 노출**하고
값을 올렸다.

### `test:cases` 신설 — 골든 10 + 에러 5

`scripts/test-cases.mts` (`npm run test:cases`). 서로 다른 지역·기간·일차마커·여행스타일·
요금의 유효 입력 10건을 **전체 파이프라인**에 통과시키고(draft + 4축 pass), 무효 입력
5건이 관문(§16.2)에서 거부되는지 본다. 골든은 AI 5회/건, 에러는 **AI 0회** — 관문이
파이프라인 시작 전에 막으므로 나쁜 입력에 예산을 쓰지 않는다.
...
|---|---|---|
| G01 제주 | 39.5초 | ✅ 턱걸이 |
| G02 부산 | 46·47·47초 | ❌ 3회 초과 → 422 |
| G03 속초 | 45.9 → 25.6초 | ✅ 재시도 |

분해 지연이 40~47초로 40초를 상습 초과했다. **`AI_TIMEOUT_MS`를 55초로 올렸다**
(`lib/types.ts` 단일 출처). `maxDuration` 60초 안이고 관측 최대 47초에 여유를 준다.
문서도 함께 올렸다 — spec §4.3 · checklist A-08 · CLAUDE.md 절대원칙 1·8 · probe 라벨.
§5.5 지연 임계값(20초)은 타임아웃과 분리돼 있으므로 건드리지 않았다.

### 55초 재실행 — 골든 8/10 · 에러 5/5 (통과 13 · 실패 2 · 1596초)

| 골든 | decompose | 결과 |
|---|---|---|
| G01 제주 4일·자연 | 44.4초 | ✅ |
| G02 부산 3일·미식 | 58·63.6 → 66.5초 | ✅ 재시도 |
| G03 속초 2일·휴양 | 74.6 → 27.4초 | ✅ 재시도 |
| G04 전주 5일·도심 | 57·58.7 → 47.1초 | ✅ 재시도(page도) |
| G05 여수 3일·문화역사 | 43.0초 | ✅ |
| G06 강릉 5일·액티비티 | 60 → 66초 | ❌ **500** |
| G07 경주 4일·자연 | — | ❌ 등록 400 |
| G08 통영 2일·휴양 | 58.2초 | ✅ (brochure 재시도) |
| G09 안동 4일·문화역사 | 24.8초 | ✅ |
| G10 서울 3일·도심 | 45.4초 | ✅ |

에러 5종(행사명 하한·마커 없음·역날짜·숙박 0건·구분 열거값)은 **전부 400 + 지정 칸**에서 거부됐다.

### 두 실패는 타임아웃도 제품 결함도 아니다

- **G07** — 픽스처 오타. `가격_아동:'0원'`을 보냈는데 `buildFormInput`이 `원`을 덧붙여
  `'0원원'`이 돼 관문 400. `'0'`(숫자만)으로 고쳤다. 요금은 폼이 숫자를 받고 서버가 `원`을 붙인다.
- **G06** — **Supabase 일시 네트워크 장애.** dev 로그: `상품 갱신 실패: TypeError: fetch failed` —
  66초 걸린 느린 분해 뒤 결과를 DB에 쓰려는데 fetch가 실패했다. 디스크(4.6GB 여유)·
  타임아웃(409 아님)·코드가 아니라 **환경**이다. 500은 §11.6 재시도 대상이 아니라
  (클라이언트는 `409 retry`만 재시도) 클라이언트가 포기했다 — 500(DB 실패)을 무한
  재시도하지 않는 것은 설계대로다.

### 남는 관측 — AI 주 경로 분해 지연이 시간대별로 크게 흔들린다

같은 날 안에서 분해가 **19초(probe) → 45~47초(첫 스위트) → 58~74초(재실행)**로 움직였다.
55초도 초과하는 호출이 나오지만(58·63·66·74초) **재시도가 받아낸다** — 40초에서 G02가
3회 소진해 죽던 것과 갈린 지점이 그것이다. 저녁 시간대(재실행)가 가장 느렸다. **데모(8/14)
시간대의 지연은 다시 재야 하고, 55초에서도 분해 재시도 1~2회는 정상이다** — 대본 3:00
안에 드는지는 그 시간대에 예열 후 재측정한다.

---

## 10.8 사용자 브라우저 테스트에서 나온 결함 3건 (2026-08-13 저녁)

기획자가 `/new`에서 직접 써 보고 나온 것들이다. **셋 다 스위트가 통과하는 상태에서
나왔다** — 자연어 초안의 클라이언트·파서 경로가 스크립트로 덜 덮여 있던 자리다.

### ① 대시 없는 라벨을 파서가 못 알아봐 숙소·상점이 비었다 (커밋 `34ceb17`)

`freeform-parse`가 라벨을 `-`/`·`/`*` **불릿으로 시작할 때만** 인식했다. 기획자가
플레이스홀더의 대시를 빼고 `숙박:`·`숙소:`·`카페:`처럼 적으면 라벨로 안 잡혀 그 블록의
항목이 **출처(분류 신호) 없이** 흐르고, 괄호(주소)까지 없으면 후보로 **아예 추출되지
않아** 숙소·상점이 빈 채로 왔다. 모든 요소가 같은 라벨 감지를 쓰므로 상점·장소·서술
필드가 전부 동일하게 실패했다. 불릿을 선택으로 내리되 **불릿 없으면 아는 낱말일 때만**
라벨로 본다(`isLabelWord`). 곁가지로 대시 없는 `여행일정: 4박5일`의 「4박5일」이 장소로
새던 것도 함께 막았다.

### ② 사람이 쓴 구체적 일정이 「자유 일정」으로 덮였다

`assembleDraft`가 일정원문을 **장소 블록 후보로만** 조립했다. 기획자가 `일정:` 블록에
「1일차: 오전 공항 도착, 오후 올레 7코스…」처럼 직접 써도 그 서술을 버리고 「1일: 자유
일정」으로 채웠다. `일정` 블록에 일차 마커가 있으면(§6.3의 6종) 그 본문을 그대로
`일정원문`으로 쓴다 — 사람이 쓴 일정이 AI 배분보다 정확하다(§9). `4박5일`처럼 마커 없이
기간만 적힌 블록은 거른다(`hasDayMarker`가 「5일」을 마커로 오인하므로 기간 표현을 먼저
뺀 뒤 검사). 실측: 대시 없는 메모로 숙박 2곳 채워지고 상세 일정이 origin=input으로 보존됨.

### ③ 행사 기간이 여행 기간과 분리되지 않았다 (spec 2.7 → 신규 필드)

「행사정보」 안의 `여행기간`이 곧 일정 일수를 정하는데, **행사(축제) 자체의 날짜와 실제
여행 날짜가 다른 경우**(제주올레걷기축제 11.05~11.07 · 여행 11.04~11.08)를 담을 자리가
없었다(§10.4 이전 `message.txt`가 이미 짚은 구멍). `행사기간_시작`·`행사기간_종료`
**선택 2필드**를 신설했다.

| 결정 | 값 |
|---|---|
| 여행 일수·일정 결정 | **여행기간** (그대로). 행사기간은 표시용 |
| 필수 여부 | **선택** — 둘 다 비면 빈 문자열, 한쪽만 채우면 관문 400 |
| 빈 값일 때 | 페이지·검토 화면에 **표시하지 않는다**(렌더러가 뺀다) |
| 폼 | 「행사정보」에 행사기간 · 「여행 기간」을 **별도 그룹**으로 분리 |

**검증을 균일하게 유지한 방법이 핵심이다.** 빈 행사기간도 confirmed_data·소개서·페이지에
`''`로 **항상 싣고**(`assertFactsUnchanged`가 `''` vs `''`를 통과), 렌더러에서만 뺀다.
그래서 `source` 맵·3차 조인·구조 검사가 조건 분기 없이 그대로 돈다. `여행기간`의 기계
경로(정규화·결합·overview·summary·0차 결합 검사)를 그대로 미러링했다.

파급: `types`·`form-validation`·`normalize`·`axis0`·`brochure`·`page`·`form.tsx`·
`sections.tsx`(공개 페이지 렌더)·`admin/products/[id]`(검토 화면 렌더)·`spec.md`. 자연어
초안은 행사기간을 **채우지 않는다** — 메모의 행사 날짜는 연도가 없는 경우가 많아
추정하지 않고 사람이 폼에서 넣는다(§7.5).

### 실측 (2026-08-13 저녁)

| 항목 | 값 |
|---|---|
| `test:policy` | **310 · 0** (대시·상세일정·행사기간 회귀 16건 추가) |
| `test:harness` | 181 · 0 · 미구현 0 |
| `verify:artifacts` | **226 · 0** (옛 form_input을 `coerceFormInput`으로 올려 크래시 제거) |
| 행사기간 관통(AI) | **10 · 0** — 행사기간(11.05~07)·여행기간(11.04~08)이 4축 통과 후 공개 페이지에 **둘 다** 표시 |
| `test:demo` | (빈 행사기간 · §20 관통 · 재검증) |

⚠️ **DB 정리**: 2.6 형식(단일 객체 숙박) 옛 테스트 상품 8건을 지웠다 — 배열 전제
검사(`verify:artifacts` 등)가 그 상품에서 터졌다. 코드 결함이 아니라 잔재 데이터다.
